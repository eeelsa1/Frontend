...
//console.log(setGPA()); //顯示undifined

//先複製第一個form 清除上面的內容
let newform = document.querySelector("form");
let addButton = document.querySelector(".plus-btn");

/*let savecredit = document.querySelectorAll(".class-credit");
let saveselect = document.querySelectorAll("select");*/

addButton.addEventListener("click", () => {
  newform = newform.cloneNode(true);
  newform.querySelectorAll("input, select").forEach(function (input) {
    input.value = ""; //對文本輸入匡和其他輸入類型進行清除，使其為空 字符串
  });
  document.querySelector(".all-inputs").appendChild(newform);

  /*function TrashSetGPA() {
    //let formlength = document.querySelectorAll("form").length;
    //let credits = document.querySelectorAll(".class-credit");
    ...
    for (let i = 0; i < credits.length; i++) {
      if (!isNaN(savecredit[i].valueAsNumber)) {
        creditSum += savecredit[i].valueAsNumber;
        sum += savecredit[i].valueAsNumber * converter(saveselect[i].value);
      }
    }
